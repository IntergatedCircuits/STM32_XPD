...
{
#ifdef RCC_BB
    __IO uint32_t *pulLPEN = PERIPH_BB(&RCC->AHB1LPENR.w);
    pulLPEN[ePeriphPos] = 1;
#else
    __IO uint32_t *pulLPENR = &RCC->AHB1LPENR.w + PPOS.regIndex;
    SET_BIT(*pulLPENR, 1 << PPOS.bitIndex);
#endif
}
...
#ifdef RCC_BB
    __IO uint32_t *pulLPEN = PERIPH_BB(&RCC->AHB1LPENR.w);
    pulLPEN[ePeriphPos] = 0;
#else
    __IO uint32_t *pulLPENR = &RCC->AHB1LPENR.w + PPOS.regIndex;
    CLEAR_BIT(*pulLPENR, 1 << PPOS.bitIndex);
#endif
}
