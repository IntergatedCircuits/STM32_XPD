...
{
#ifdef RCC_BB
    __IO uint32_t *pLPEN = PERIPH_BB(&RCC->AHB1LPENR.w);
    pLPEN[PeriphPos] = 1;
#else
    __IO uint32_t *pLPENR = &RCC->AHB1LPENR.w + PPOS.regIndex;
    SET_BIT(*pLPENR, 1 << PPOS.bitIndex);
#endif
}
...
#else
    __IO uint32_t *pLPENR = &RCC->AHB1LPENR.w + PPOS.regIndex;
    CLEAR_BIT(*pLPENR, 1 << PPOS.bitIndex);
#endif
}
